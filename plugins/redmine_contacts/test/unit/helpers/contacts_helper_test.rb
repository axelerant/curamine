...
class ContactsHelperTest < ActionView::TestCase
  ...
  def test_contacts_to_csv
    csv_result = contacts_to_csv(Contact.all)
    assert_match /First Name/, csv_result
    assert_match /Domoway/, csv_result
  end


  def test_contacts_to_csv_with_multivalue_custom_field
    ...
    contact.save!
    csv_result = contacts_to_csv([contact])
    assert_match /First Name/, csv_result
    assert_match /Domoway/, csv_result
